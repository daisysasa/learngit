...
(function ($)
{
    ...
    //daowenui 继承方法
    Function.prototype.daowenExtend = function (parent, overrides)
    {
    ...
    //延时加载
    Function.prototype.daowenDefer = function (o, defer, args)
    {
        var fn = this;
    ...
    // 核心对象
    window.daowen = $.daowenui = {
        version: 'V1.1.9',
        managerCount: 0,
        //组件管理器池
        managers: {},
        managerIdPrev: 'daowenui',
        //错误提示
        error: {
        ...
        remove: function (arg)
        {
            ...
            {
                delete $.daowenui.managers[arg];
            }
            else if (typeof arg == "object" && arg instanceof $.daowenui.core.Component)
            {
                delete $.daowenui.managers[arg.id];
            }
        },
        //获取daowenui对象
        //1,传入daowenui ID
        ...
        {
            idAttrName = idAttrName || "daowenuiid";
            if (typeof arg == "string" || typeof arg == "number")
            {
                return $.daowenui.managers[arg];
            }
            else if (typeof arg == "object" && arg.length)
            {
                if (!arg[0][idAttrName] && !$(arg[0]).attr(idAttrName)) return null;
                return $.daowenui.managers[arg[0][idAttrName] || $(arg[0]).attr(idAttrName)];
            }
            return null;
        },
        //根据类型查找某一个对象
        ...
        //$.fn.daowen{Plugin} 和 $.fn.daowenGet{Plugin}Manager
        //会调用这个方法,并传入作用域(this)
        //@parm [plugin]  插件名
        //@parm [args] 参数(数组)
        ...
        run: function (plugin, args, ext)
        {
            if (!plugin) return;
            ext = $.extend({
                defaultsNamespace: 'daowenDefaults',
                methodsNamespace: 'daowenMethods',
                controlNamespace: 'controls',
                idAttrName: 'daowenuiid',
                isStatic: false,
                hasElement: true,           //是否拥有element主体(比如drag、resizable等辅助性插件就不拥有)
                propertyToElemnt: null      //链接到element的属性名
            }, ext || {});
            plugin = plugin.replace(/^daowenGet/, '');
            plugin = plugin.replace(/^daowen/, '');
            if (this == null || this == window || ext.isStatic)
            {
                if (!$.daowenui.plugins[plugin])
                {
                    $.daowenui.plugins[plugin] = {
                        fn: $['daowen' + plugin],
                        isStatic: true
                    };
                }
                return new $.daowenui[ext.controlNamespace][plugin]($.extend({}, $[ext.defaultsNamespace][plugin] || {}, $[ext.defaultsNamespace][plugin + 'String'] || {}, args.length > 0 ? args[0] : {}));
            }
            if (!$.daowenui.plugins[plugin])
            {
                $.daowenui.plugins[plugin] = {
            ...
            if (/Manager$/.test(plugin)) return $.daowenui.get(this, ext.idAttrName);
            this.each(function ()
            {
                if (this[ext.idAttrName] || $(this).attr(ext.idAttrName))
                {
                    var manager = $.daowenui.get(this[ext.idAttrName] || $(this).attr(ext.idAttrName));
                ...
                if (ext.hasElement)
                {
                    new $.daowenui[ext.controlNamespace][plugin](this, p);
                }
                else
                {
                    new $.daowenui[ext.controlNamespace][plugin](p);
                }
            });
            if (this.length == 0) return null;
            if (args.length == 0) return $.daowenui.get(this, ext.idAttrName);
            if (typeof args[0] == 'object') return $.daowenui.get(this, ext.idAttrName);
            if (typeof args[0] == 'string')
            {
                var manager = $.daowenui.get(this, ext.idAttrName);
    ...
    //关联起来
    $.daowenui.defaults = $.daowenDefaults;
    $.daowenui.methods = $.daowenMethos;

    //获取daowenui对象
    //@parm [plugin]  插件名,可为空
    $.fn.daowen = function (plugin)
    {
        if (plugin)
        {
            return $.daowenui.run.call(this, plugin, arguments);
        }
        else
        {
            return $.daowenui.get(this);
        }
    };
    ...
    $.daowenui.core.Component = function (options)
    {
        //事件容器
    ...
    $.extend($.daowenui.core.Component.prototype, {
        __getType: function ()
        {
            return '$.daowenui.core.Component';
        },
        __idPrev: function ()
        {
            return 'daowenui';
    ...
    //@parm [options] 组件的参数
    $.daowenui.core.UIComponent = function (element, options)
    {
        $.daowenui.core.UIComponent.base.constructor.call(this, options);
    ...
    $.daowenui.core.UIComponent.daowenExtend($.daowenui.core.Component, {
        __getType: function ()
        {
            return '$.daowenui.core.UIComponent';
        },
        //扩展方法
        ...
        _init: function ()
        {
            ...
            if (!this.element)
            {
                this.id = this.options.id || $.daowenui.getId(this.__idPrev());
            }
            else
            {
                this.id = this.options.id || this.element.id || $.daowenui.getId(this.__idPrev());
            }
            //存入管理器池
            $.daowenui.add(this);
            ...
            //读取daowenui这个属性，并加载到参数，比如 daowenui = "width:120,heigth:100"
            var p = this.options;
            if ($(this.element).attr("daowenui"))
            {
                try
                {
                    var attroptions = $(this.element).attr("daowenui");
        ...
        _rendered: function ()
        {
            if (this.element)
            {
                $(this.element).attr("daowenuiid", this.id);
            }
        },
        //返回要转换成daowenui参数的属性,比如['url']
        attr: function ()
        {
            return [];
        },
    ...
    //表单控件基类
    $.daowenui.controls.Input = function (element, options)
    {
        $.daowenui.controls.Input.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.Input.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return '$.daowenui.controls.Input';
        },
        attr: function ()
        {
            return ['nullText'];
    ...
    $.daowenui.win = {
        ...
        mask: function (win)
        {
            function setHeight()
            {
                if (!$.daowenui.win.windowMask) return;
                var h = $(window).height() + $(window).scrollTop();
                $.daowenui.win.windowMask.height(h);
            }
            if (!this.windowMask)
            {
                this.windowMask = $("<div class='l-window-mask' style='display: block;'></div>").appendTo('body');
                $(window).bind('resize.daowenuiwin', setHeight);
        ...
        unmask: function (win)
        {
            ...
            {
                var winid = jwins.eq(i).attr("daowenuiid");
                if (win && win.id == winid) continue;
                //获取daowenui对象
                var winmanager = $.daowenui.get(winid);
        ...
        {
            var wins = $.daowenui.find($.daowenui.core.Win);
            for (var i in wins)
            {
                var w = wins[i];
                if (w == win)
    ...
    //窗口基类 window、dialog
    $.daowenui.core.Win = function (element, options)
    {
        $.daowenui.core.Win.base.constructor.call(this, element, options);
    };

    $.daowenui.core.Win.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return '$.daowenui.controls.Win';
        },
        mask: function ()
        {
            if (this.options.modal)
                $.daowenui.win.mask(this);
        },
        unmask: function ()
        {
            if (this.options.modal)
                $.daowenui.win.unmask(this);
        },
        min: function ()
        {
        },
    ...
    $.daowenui.toJSON = typeof JSON === 'object' && JSON.stringify ? JSON.stringify : function (o)
    {
        var f = function (n)
        {
            return n < 10 ? '0' + n : n;
        ...
        if (type === 'object')
        {
            ...
            {
                return $.daowenui.toJSON(o.toJSON());
            }
            ...
            if (o.constructor === Array)
            {
                ...
                {
                    pairs.push($.daowenui.toJSON(o[i]) || 'null');
                }
                return '[' + pairs.join(',') + ']';
            }
            var name, val;
...
(function ($)
{
    $.fn.daowenAccordion = function (options)
    {
        return $.daowenui.run.call(this, "daowenAccordion", arguments);
    };

    $.fn.daowenGetAccordionManager = function ()
    {
        return $.daowenui.get(this);
    };

    $.daowenDefaults.Accordion = {
        height: null,
        speed: "normal",
        changeHeightOnResize: false,
        heightDiff: 0 // 高度补差  
    };
    $.daowenMethos.Accordion = {};

    $.daowenui.controls.Accordion = function (element, options)
    {
        $.daowenui.controls.Accordion.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Accordion.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Accordion';
        },
...
(function ($)
{
    ...
    $.fn.daowenButton = function (options)
    {
        return $.daowenui.run.call(this, "daowenButton", arguments);
    };
    $.fn.daowenGetButtonManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetButtonManager", arguments);
    };

    $.daowenDefaults.Button = { width: 100, text: 'Button', disabled: false };

    $.daowenMethos.Button = {};

    $.daowenui.controls.Button = function (element, options)
    {
        $.daowenui.controls.Button.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Button.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'Button';
        },
...
(function ($)
{
    $.fn.daowenCheckBox = function (options)
    {
        return $.daowenui.run.call(this, "daowenCheckBox", arguments);
    };
    $.fn.daowenGetCheckBoxManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetCheckBoxManager", arguments);
    };
    $.daowenDefaults.CheckBox = { disabled: false };

    $.daowenMethos.CheckBox = {};

    $.daowenui.controls.CheckBox = function (element, options)
    {
        $.daowenui.controls.CheckBox.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.CheckBox.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'CheckBox';
        },
...
(function ($)
{
    ...
    $.fn.daowenComboBox = function (options)
    {
        return $.daowenui.run.call(this, "daowenComboBox", arguments);
    };

    $.fn.daowenGetComboBoxManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetComboBoxManager", arguments);
    };

    $.daowenDefaults.ComboBox = {
        resize: true,           //是否调整大小
    ...
    //扩展方法
    $.daowenMethos.ComboBox = $.daowenMethos.ComboBox || {};


    $.daowenui.controls.ComboBox = function (element, options)
    {
        $.daowenui.controls.ComboBox.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.ComboBox.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'ComboBox';
        },
        _extendMethods: function ()
        {
            return $.daowenMethos.ComboBox;
        },
        _init: function ()
        {
            $.daowenui.controls.ComboBox.base._init.call(this);
            var p = this.options;
            if (p.columns)
            {
        ...
        {
            //调整大小支持
            if (resize && $.fn.daowenResizable)
            {
                var g = this;
                g.selectBox.daowenResizable({ handles: 'se,s,e', onStartResize: function ()
                {
                    g.resizing = true;
        ...
        setData: function (data)
        {
            ...
            //自定义复选框支持
            if (p.isShowCheckBox && $.fn.daowenCheckBox)
            {
                $("table input:checkbox", g.selectBox).daowenCheckBox();
            }
        ...
        setTree: function (tree)
        {
            ...
            g.tree.daowenTree(tree);
            g.treeManager = g.tree.daowenGetTreeManager();
        },
        ...
        setGrid: function (grid)
        {
            ...
            grid.heightDiff = -2;
            grid.InWindow = false;
            g.gridManager = g.grid.daowenGrid(grid);
    ...
    $.daowenui.controls.ComboBox.prototype.setValue = $.daowenui.controls.ComboBox.prototype.selectValue;
    //设置文本框和隐藏控件的值
    $.daowenui.controls.ComboBox.prototype.setInputValue = $.daowenui.controls.ComboBox.prototype._changeValue;
...
(function ($)
{
    $.fn.daowenDateEditor = function ()
    {
        return $.daowenui.run.call(this, "daowenDateEditor", arguments);
    };

    $.fn.daowenGetDateEditorManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetDateEditorManager", arguments);
    };

    $.daowenDefaults.DateEditor = {
        format: "yyyy-MM-dd hh:mm",
        showTime: false,
        onChangeDate: false,
        absolute: true  //选择框是否在附加到body,并绝对定位
    };
    $.daowenDefaults.DateEditorString = {
        dayMessage: ["日", "一", "二", "三", "四", "五", "六"],
        monthMessage: ["一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "十二月"],
        todayMessage: "今天",
        closeMessage: "关闭"
    };
    $.daowenMethos.DateEditor = {};

    $.daowenui.controls.DateEditor = function (element, options)
    {
        $.daowenui.controls.DateEditor.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.DateEditor.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'DateEditor';
        },
        ...
        _extendMethods: function ()
        {
            return $.daowenMethos.DateEditor;
        },
        _render: function ()
        {
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.daowenDialog = function ()
    {
        return l.run.call(null, "daowenDialog", arguments, { isStatic: true });
    };

    //dialog 图片文件夹的路径 预加载
    $.daowenui.DialogImagePath = "../../lib/daowenUI/skins/Aqua/images/win/";
    ...
    $.daowenMethos.Dialog = $.daowenMethos.Dialog || {};
    ...
    l.controls.Dialog.daowenExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Dialog';
        },
        ...
        _render: function ()
        {
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = p.name ? p.name : "daowenwindow" + new Date().getTime();
        ...
        show: function ()
        {
            ...
            //前端显示 
            $.daowenui.win.setFront.daowenDefer($.daowenui.win, 100, [g]);
        },
        setUrl: function (url)
        {
            this._setUrl(url);
        ...
        _applyDrag: function ()
        {
            var g = this, p = this.options;
            if ($.fn.daowenDrag)
                g.draggable = g.dialog.daowenDrag({ handler: '.l-dialog-title', animate: false,
                    ...
                    onStopDrag: function ()
                    {
                        if (p.target)
                        {
                            var triggers1 = l.find($.daowenui.controls.DateEditor);
                            var triggers2 = l.find($.daowenui.controls.ComboBox);
                            //更新所有下拉选择框的位置
        ...
        _onReisze: function ()
        {
            ...
            if (p.target)
            {
                var manager = $(p.target).daowen();
                if (!manager) manager = $(p.target).find(":first").daowen();
                if (!manager) return;
        ...
        {
            var g = this, p = this.options;
            if ($.fn.daowenResizable)
            {
                g.resizable = g.dialog.daowenResizable({
    ...
    $.daowenDialog.open = function (p)
    {
        return $.daowenDialog(p);
    };
    $.daowenDialog.close = function ()
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        for (var i in dialogs)
        {
            var d = dialogs[i];
            d.destroy.daowenDefer(d, 5);
        }
        l.win.unmask();
    };
    $.daowenDialog.show = function (p)
    {
        var dialogs = l.find(l.controls.Dialog.prototype.__getType());
        if (dialogs.length)
        {
    ...
    $.daowenDialog.tip = function (options)
    {
        options = $.extend({
            showType: 'slide',
            width: 240,
        ...
        return $.daowenDialog.open(options);
    };
    $.daowenDialog.alert = function (content, title, type, callback)
    {
        content = content || "";
        ...
        p = {
            content: content,
            buttons: [{ text: $.daowenDefaults.DialogString.ok, onclick: btnclick}]
        };
    ...
    $.daowenDialog.confirm = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.daowenDefaults.DialogString.yes, onclick: btnclick, type: 'ok' }, { text: $.daowenDefaults.DialogString.no, onclick: btnclick, type: 'no'}]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.daowenDialog.warning = function (content, title, callback)
    {
        if (typeof (title) == "function")
        {
            callback = title;
        ...
        p = {
            type: 'question',
            content: content,
            buttons: [{ text: $.daowenDefaults.DialogString.yes, onclick: btnclick, type: 'yes' }, { text: $.daowenDefaults.DialogString.no, onclick: btnclick, type: 'no' }, { text: $.daowenDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel'}]
        };
        if (typeof (title) == "string" && title != "") p.title = title;
        $.extend(p, {
            showMax: false,
    ...
    $.daowenDialog.waitting = function (title)
    {
        title = title || $.daowenDefaults.Dialog.waittingMessage;
        return $.daowenDialog.open({ cls: 'l-dialog-waittingdialog', type: 'none', content: '<div style="padding:4px">' + title + '</div>', allowClose: false });
    };
    $.daowenDialog.closeWaitting = function ()
    {
    ...
    $.daowenDialog.success = function (content, title, onBtnClick)
    {
        return $.daowenDialog.alert(content, title, 'success', onBtnClick);
    };
    $.daowenDialog.error = function (content, title, onBtnClick)
    {
        return $.daowenDialog.alert(content, title, 'error', onBtnClick);
    };
    $.daowenDialog.warn = function (content, title, onBtnClick)
    {
        return $.daowenDialog.alert(content, title, 'warn', onBtnClick);
    };
    $.daowenDialog.question = function (content, title)
    {
        return $.daowenDialog.alert(content, title, 'question');
    };


    $.daowenDialog.prompt = function (title, value, multi, callback)
    {
        ...
        p = {
            ...
            target: target,
            width: 320,
            buttons: [{ text: $.daowenDefaults.DialogString.ok, onclick: btnclick, type: 'yes' }, { text: $.daowenDefaults.DialogString.cancel, onclick: btnclick, type: 'cancel'}]
        };
        return $.daowenDialog(p);
    };
...
(function ($)
{
    ...
    $.fn.daowenDrag = function (options)
    {
        return l.run.call(this, "daowenDrag", arguments,
        {
            idAttrName: 'daowenuidragid', hasElement: false, propertyToElemnt: 'target'
        }
        );
    };

    $.fn.daowenGetDragManager = function ()
    {
        return l.run.call(this, "daowenGetDragManager", arguments,
        {
            idAttrName: 'daowenuidragid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    l.controls.Drag.daowenExtend(l.core.UIComponent, {
        __getType: function ()
        {
            return 'Drag';
        },
        ...
        _rendered: function ()
        {
            this.options.target.daowenuidragid = this.id;
        },
        _start: function (e)
        {
            var g = this, p = this.options;
...
(function ($)
{
    $.fn.daowenEasyTab = function ()
    {
        return $.daowenui.run.call(this, "daowenEasyTab", arguments);
    };
    $.fn.daowenGetEasyTabManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetEasyTabManager", arguments);
    };

    $.daowenDefaults.EasyTab = {};

    $.daowenMethos.EasyTab = {};

    $.daowenui.controls.EasyTab = function (element, options)
    {
        $.daowenui.controls.EasyTab.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.EasyTab.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'EasyTab';
        },
...
(function ($)
{
    $.fn.daowenFilter = function ()
    {
        return $.daowenui.run.call(this, "daowenFilter", arguments);
    };

    $.fn.daowenGetFilterManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetFilterManager", arguments);
    };

    $.daowenDefaults.Filter = {
        //字段列表
        fields: [],
        //字段类型 - 运算符 的对应关系
        operators: {},
    ...
    $.daowenDefaults.Filter.operators['string'] =
    $.daowenDefaults.Filter.operators['text'] =
    ["equal", "notequal", "startwith", "endwith", "like", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];

    $.daowenDefaults.Filter.operators['number'] =
    $.daowenDefaults.Filter.operators['int'] =
    $.daowenDefaults.Filter.operators['float'] =
    $.daowenDefaults.Filter.operators['date'] =
    ["equal", "notequal", "greater", "greaterorequal", "less", "lessorequal", "in", "notin"];

    $.daowenDefaults.Filter.editors['string'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            input.daowenTextBox(field.editor.options || {});
            return input;
        },
        ...
        {
            return input.daowen('option', 'value');
        },
        destroy: function (input)
        {
            input.daowen('destroy');
        }
    };

    $.daowenDefaults.Filter.editors['date'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            container.append(input);
            input.daowenDateEditor(field.editor.options || {});
            return input;
        },
        setValue: function (input, value)
        {
            input.daowen('option', 'value', value);
        },
    ...
    $.daowenDefaults.Filter.editors['number'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
            ...
            input.daowenSpinner($.extend(options, field.editor.options || {}));
            return input;
        },
        setValue: function (input, value)
        {
    ...
    $.daowenDefaults.Filter.editors['combobox'] =
    {
        create: function (container, field)
        {
            var input = $("<input type='text'/>");
    ...
    //过滤器组件
    $.daowenui.controls.Filter = function (element, options)
    {
        $.daowenui.controls.Filter.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.Filter.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Filter'
        },
        ...
        _init: function ()
        {
            $.daowenui.controls.Filter.base._init.call(this);
        },
        _render: function ()
        {
            var g = this, p = this.options;
...
(function ($)
{
    $.fn.daowenForm = function ()
    {
        return $.daowenui.run.call(this, "daowenForm", arguments);
    };

    $.daowenDefaults = $.daowenDefaults || {};
    $.daowenDefaults.Form = {
        //控件宽度
        inputWidth: 180,
        //标签宽度
    ...
    $.daowenDefaults.Form.editorBulider = function (jinput)
    {
        ...
        else if (jinput.is(":text") || jinput.is(":password"))
        {
            ...
            switch (ltype)
            {
                ...
                case "combobox":
                    ...
                    break;
                case "spinner":
                    jinput.daowenSpinner(inputOptions);
                    break;
                case "date":
                    jinput.daowenDateEditor(inputOptions);
                    break;
                case "float":
                case "number":
                    inputOptions.number = true;
                    jinput.daowenTextBox(inputOptions);
                    break;
                case "int":
                case "digits":
        ...
        {
            jinput.daowenRadio(inputOptions);
        }
        else if (jinput.is(":checkbox"))
        {
            jinput.daowenCheckBox(inputOptions);
        }
    ...
    //表单组件
    $.daowenui.controls.Form = function (element, options)
    {
        $.daowenui.controls.Form.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.Form.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Form'
        },
        ...
        _init: function ()
        {
            $.daowenui.controls.Form.base._init.call(this);
        },
        _render: function ()
        {
            var g = this, p = this.options;
        ...
        _buliderControl: function (field)
        {
            ...
            out.push(" daowenui='" + p.toJSON(fieldOptions) + "' ");
            //验证参数
            if (field.validate)
            {
...
(function ($)
{
    ...
    $.fn.daowenGrid = function (options)
    {
        return $.daowenui.run.call(this, "daowenGrid", arguments);
    };

    $.fn.daowenGetGridManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetGridManager", arguments);
    };
    ...
    $.daowenDefaults.Grid = {
        ...
        switchPageSizeApplyComboBox: false,     //切换每页记录数是否应用daowenComboBox
        ...
        toolbar: null,                           //工具条,参数同 daowenToolbar的
        headerImg: null                        //表格头部图标
    };
    $.daowenDefaults.GridString = {
        errorMessage: '发生错误',
    ...
    //接口方法扩展
    $.daowenMethos.Grid = $.daowenMethos.Grid || {};

    //排序器扩展
    $.daowenDefaults.Grid.sorters = $.daowenDefaults.Grid.sorters || {};

    //格式化器扩展
    $.daowenDefaults.Grid.formatters = $.daowenDefaults.Grid.formatters || {};

    //编辑器扩展
    $.daowenDefaults.Grid.editors = $.daowenDefaults.Grid.editors || {};


    $.daowenDefaults.Grid.sorters['date'] = function (val1, val2)
    {
        return val1 < val2 ? -1 : val1 > val2 ? 1 : 0;
    };
    $.daowenDefaults.Grid.sorters['int'] = function (val1, val2)
    {
        return parseInt(val1) < parseInt(val2) ? -1 : parseInt(val1) > parseInt(val2) ? 1 : 0;
    };
    $.daowenDefaults.Grid.sorters['float'] = function (val1, val2)
    {
        return parseFloat(val1) < parseFloat(val2) ? -1 : parseFloat(val1) > parseFloat(val2) ? 1 : 0;
    };
    $.daowenDefaults.Grid.sorters['string'] = function (val1, val2)
    {
        return val1.localeCompare(val2);
    };


    $.daowenDefaults.Grid.formatters['date'] = function (value, column)
    {
    ...
    $.daowenDefaults.Grid.editors['date'] =
    {
        create: function (container, editParm)
        {
            ...
            input.daowenDateEditor(options);
            return input;
        },
        ...
        {
            input.daowen('option', 'width', width);
            input.daowen('option', 'height', height);
        },
    ...
    $.daowenDefaults.Grid.editors['select'] =
     $.daowenDefaults.Grid.editors['combobox'] =
     {
         create: function (container, editParm)
         {
             var column = editParm.column;
    ...
    $.daowenDefaults.Grid.editors['int'] =
     $.daowenDefaults.Grid.editors['float'] =
     $.daowenDefaults.Grid.editors['spinner'] =
     {
         create: function (container, editParm)
         {
             var column = editParm.column;
    ...
    $.daowenDefaults.Grid.editors['string'] =
     $.daowenDefaults.Grid.editors['text'] = {
         create: function (container, editParm)
         {
    ...
    $.daowenDefaults.Grid.editors['chk'] = $.daowenDefaults.Grid.editors['checkbox'] = {
        create: function (container, editParm)
        {
            var input = $("<input type='checkbox' />");
            container.append(input);
            input.daowenCheckBox();
            return input;
        },
        getValue: function (input, editParm)
        {
    ...
    $.daowenui.controls.Grid = function (element, options)
    {
        $.daowenui.controls.Grid.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.Grid.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return '$.daowenui.controls.Grid';
        },
        __idPrev: function ()
        {
            return 'grid';
        ...
        _extendMethods: function ()
        {
            return $.daowenMethos.Grid;
        },
        _init: function ()
        {
            $.daowenui.controls.Grid.base._init.call(this);
            var g = this, p = this.options;
        ...
        loadData: function (loadDataParm)
        {
            ...
            else
            {
                ...
                //g.loadServerData.daowenDefer(g, 10, [param, clause]);
            }
            g.loading = false;
        },
        loadServerData: function (param, clause)
        {
            ...
            var ajaxOptions = {
                ...
                success: function (data)
                {
                    ...
                    g._showData.daowenDefer(g, 10, [g.currentData]);
                },
                complete: function ()
                {
                    ...
                    {
                        g.toggleLoading.daowenDefer(g, 10, [false]);
                    }
                },
        ...
        deleteSelectedRow: function ()
        {
            ...
            for (var i in this.selected)
            {
                ...
                if (o['__id'] in this.records)
                    this._deleteData.daowenDefer(this, 10, [o]);
            }
            this.reRender.daowenDefer(this, 20);
        },
        ...
        */
        ...
        updateRow: function (rowDom, newRowData)
        {
            ...
            if (rowdata[p.statusName] != 'add')
                rowdata[p.statusName] = 'update';
            g.reRender.daowenDefer(g, 10, [{ rowdata: rowdata}]);
            return rowdata;
        },
        ...
        toggleCol: function (columnparm, visible, toggleByPopup)
        {
            ...
            if (!toggleByPopup)
            {
                $(':checkbox[columnindex=' + columnindex + "]", g.popup).each(function ()
                {
                    ...
                    if ($.fn.daowenCheckBox)
                    {
                        var checkboxmanager = $(this).daowenGetCheckBoxManager();
        ...
        _initBuildHeader: function ()
        {
            ...
            {
                if ($.fn.daowenToolBar)
                    g.toolbarManager = g.topbar.daowenToolBar(p.toolbar);
            }
            else
            {
                g.topbar.remove();
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (!column.issystem && column['__leaf'] && column.resizable !== false && $.fn.daowenResizable)
            {
                g.colResizable[column['__id']] = jcell.daowenResizable({ handles: 'e',
        ...
        _initBuildPopup: function ()
        {
            ...
            if ($.fn.daowenCheckBox)
            {
                $('input:checkbox', g.popup).daowenCheckBox(
                {
        ...
        _initFootbar: function ()
        {
            ...
            if (p.usePager)
            {
                ...
                if (p.switchPageSizeApplyComboBox && $.fn.daowenComboBox)
                {
                    $(".l-bar-selectpagesize select", g.toolbar).daowenComboBox(
                    {
        ...
        _setEvent: function ()
        {
            ...
            //列拖拽支持
            if ($.fn.daowenDrag && p.colDraggable)
            {
                ...
                g.gridheader.add(g.f.gridheader).daowenDrag({ revert: true, animate: false,
                    proxyX: 0, proxyY: 0,
                    ...
                    onStopDrag: function (current, e)
                    {
                        ...
                        if (g.colDropIn != -1)
                        {
                            g.changeCol.daowenDefer(g, 0, [column, g.colDropIn, g.colDropDir == "right"]);
                            g.colDropIn = -1;
                        }
                        g.colDroptip.hide();
                        this.set('cursor', 'default');
            ...
            //行拖拽支持
            if ($.fn.daowenDrag && p.rowDraggable)
            {
                g.rowDroptip = $("<div class='l-drag-rowdroptip' style='display:none'></div>").appendTo('body');
                g.gridbody.add(g.f.gridbody).daowenDrag({ revert: true, animate: false,
                    proxyX: 0, proxyY: 0,
                    proxy: function (draggable, e)
                    {
                        var src = g._getSrcElementByEvent(e);
        ...
        _onClick: function (e)
        {
            ...
            if (src.out)
            {
                if (g.editor.editing && !$.daowenui.win.masking) g.endEdit();
    ...
    $.daowenui.controls.Grid.prototype.enabledTotal = $.daowenui.controls.Grid.prototype.isTotalSummary;
    $.daowenui.controls.Grid.prototype.add = $.daowenui.controls.Grid.prototype.addRow;
    $.daowenui.controls.Grid.prototype.update = $.daowenui.controls.Grid.prototype.updateRow;
    $.daowenui.controls.Grid.prototype.append = $.daowenui.controls.Grid.prototype.appendRow;
    $.daowenui.controls.Grid.prototype.getSelected = $.daowenui.controls.Grid.prototype.getSelectedRow;
    $.daowenui.controls.Grid.prototype.getSelecteds = $.daowenui.controls.Grid.prototype.getSelectedRows;
    $.daowenui.controls.Grid.prototype.getCheckedRows = $.daowenui.controls.Grid.prototype.getSelectedRows;
    $.daowenui.controls.Grid.prototype.getCheckedRowObjs = $.daowenui.controls.Grid.prototype.getSelectedRowObjs;
    $.daowenui.controls.Grid.prototype.setOptions = $.daowenui.controls.Grid.prototype.set;
...
(function ($)
{
    $.fn.daowenLayout = function (options)
    {
        return $.daowenui.run.call(this, "daowenLayout", arguments);
    };

    $.fn.daowenGetLayoutManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetLayoutManager", arguments);
    };


    $.daowenDefaults.Layout = {
        topHeight: 50,
        bottomHeight: 50,
        leftWidth: 110,
        centerWidth: 300,
    ...
    $.daowenui.controls.Layout = function (element, options)
    {
        $.daowenui.controls.Layout.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Layout.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Layout';
        },
...
(function ($)
{
    $.daowenMenu = function (options)
    {
        return $.daowenui.run.call(null, "daowenMenu", arguments);
    };
    ...
    $.daowenui.controls.Menu = function (options)
    {
        $.daowenui.controls.Menu.base.constructor.call(this, null, options);
    };
    $.daowenui.controls.Menu.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Menu';
        },
        ...
        addItem: function (item, menu)
        {
            ...
            ditem.attr("daowenuimenutemid", ++g.menuItemCount);
            ...
            if (item.children)
            {
                ...
                var newmenu = g.createMenu(ditem.attr("daowenuimenutemid"));
                g.menus[ditem.attr("daowenuimenutemid")] = newmenu;
            ...
            ditem.hover(function ()
            {
                ...
                if (item.children)
                {
                    var daowenuimenutemid = $(this).attr("daowenuimenutemid");
                    if (!daowenuimenutemid) return;
                    if (g.menus[daowenuimenutemid])
                    {
                        g.show({ top: itemtop, left: $(this).offset().left + $(this).width() - 5 }, g.menus[daowenuimenutemid]);
        ...
        hideAllSubMenu: function (menu)
        {
            ...
            $("> .l-menu-item", menu.items).each(function ()
            {
                if ($("> .l-menu-item-arrow", this).length > 0)
                {
                    var daowenuimenutemid = $(this).attr("daowenuimenutemid");
                    if (!daowenuimenutemid) return;
                    g.menus[daowenuimenutemid] && g.hide(g.menus[daowenuimenutemid]);
                }
            });
        ...
        createMenu: function (parentMenuItemID)
        {
            ...
            parentMenuItemID && menu.attr("daowenuiparentmenuitemid", parentMenuItemID);
    ...
    //旧写法保留
    $.daowenui.controls.Menu.prototype.setEnable = $.daowenui.controls.Menu.prototype.setEnabled;
    $.daowenui.controls.Menu.prototype.setDisable = $.daowenui.controls.Menu.prototype.setDisabled;
...
(function ($)
{
    $.fn.daowenMenuBar = function (options)
    {
        return $.daowenui.run.call(this, "daowenMenuBar", arguments);
    };
    $.fn.daowenGetMenuBarManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetMenuBarManager", arguments);
    };

    $.daowenDefaults.MenuBar = {};

    $.daowenMethos.MenuBar = {};

    $.daowenui.controls.MenuBar = function (element, options)
    {
        $.daowenui.controls.MenuBar.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.MenuBar.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'MenuBar';
        },
...
(function ($)
{
    ...
    $.daowenMessageBox = function (options)
    {
        return $.daowenui.run.call(null, "daowenMessageBox", arguments, { isStatic: true });
    };


    $.daowenDefaults.MessageBox = {
        isDrag: true
    };

    $.daowenMethos.MessageBox = {};

    $.daowenui.controls.MessageBox = function (options)
    {
        $.daowenui.controls.MessageBox.base.constructor.call(this, null, options);
    };
    $.daowenui.controls.MessageBox.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'MessageBox';
        },
        ...
        _extendMethods: function ()
        {
            return $.daowenMethos.MessageBox;
        },
        _render: function ()
        {
            var g = this, p = this.options;
        ...
        {
            var g = this, p = this.options;
            if (p.isDrag && $.fn.daowenDrag)
                g.messageBox.daowenDrag({ handler: '.l-messagebox-title-inner', animate: false });
        },
        _setImage: function ()
        {
            var g = this, p = this.options;
    ...
    $.daowenMessageBox.show = function (p)
    {
        return $.daowenMessageBox(p);
    };
    $.daowenMessageBox.alert = function (title, content, type, onBtnClick)
    {
        title = title || "";
    ...
    $.daowenMessageBox.confirm = function (title, content, callback)
    {
        var onclick = function (item, index, messageBox)
        {
            messageBox.close();
    ...
    $.daowenMessageBox.success = function (title, content, onBtnClick)
    {
        return $.daowenMessageBox.alert(title, content, 'success', onBtnClick);
    };
    $.daowenMessageBox.error = function (title, content, onBtnClick)
    {
        return $.daowenMessageBox.alert(title, content, 'error', onBtnClick);
    };
    $.daowenMessageBox.warn = function (title, content, onBtnClick)
    {
        return $.daowenMessageBox.alert(title, content, 'warn', onBtnClick);
    };
    $.daowenMessageBox.question = function (title, content)
    {
        return $.daowenMessageBox.alert(title, content, 'question');
    };
...
(function ($)
{
    ...
    $.fn.daowenRadio = function ()
    {
        return $.daowenui.run.call(this, "daowenRadio", arguments);
    };

    $.fn.daowenGetRadioManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetRadioManager", arguments);
    };

    $.daowenDefaults.Radio = { disabled: false };

    $.daowenMethos.Radio = {};

    $.daowenui.controls.Radio = function (element, options)
    {
        $.daowenui.controls.Radio.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Radio.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'Radio';
        },
...
(function ($)
{
    $.fn.daowenResizable = function (options)
    {
        return $.daowenui.run.call(this, "daowenResizable", arguments,
        {
            idAttrName: 'daowenuiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };

    $.fn.daowenGetResizableManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetResizableManager", arguments,
        {
            idAttrName: 'daowenuiresizableid', hasElement: false, propertyToElemnt: 'target'
        });
    };
    ...
    $.daowenui.controls.Resizable = function (options)
    {
        $.daowenui.controls.Resizable.base.constructor.call(this, null, options);
    };

    $.daowenui.controls.Resizable.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Resizable';
        },
        ...
        _render: function ()
        {
            ...
            g.target.mousemove(function (e)
            {
                ...
                if (p.target.daowenuidragid)
                {
                    var drag = $.daowenui.get(p.target.daowenuidragid);
                    if (drag && g.dir)
                    {
        ...
        _rendered: function ()
        {
            this.options.target.daowenuiresizableid = this.id;
        },
        _getDir: function (e)
        {
            var g = this, p = this.options;
...
(function ($)
{
    $.fn.daowenSpinner = function ()
    {
        return $.daowenui.run.call(this, "daowenSpinner", arguments);
    };
    $.fn.daowenGetSpinnerManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetSpinnerManager", arguments);
    };

    $.daowenDefaults.Spinner = {
    ...
    $.daowenMethos.Spinner = {};

    $.daowenui.controls.Spinner = function (element, options)
    {
        $.daowenui.controls.Spinner.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Spinner.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'Spinner';
        },
        ...
        _extendMethods: function ()
        {
            return $.daowenMethos.Spinner;
        },
        _init: function ()
        {
            $.daowenui.controls.Spinner.base._init.call(this);
            var p = this.options;
            if (p.type == 'float')
            {
                p.step = 0.1;
...
(function ($)
{
    ...
    $.fn.daowenTab = function (options)
    {
        return $.daowenui.run.call(this, "daowenTab", arguments);
    };

    $.fn.daowenGetTabManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetTabManager", arguments);
    };
    ...
    $.daowenui.controls.Tab = function (element, options)
    {
        $.daowenui.controls.Tab.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.Tab.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Tab';
        },
        ...
        _render: function ()
        {
            ...
            if (p.contextmenu && $.daowenMenu)
            {
                g.tab.menu = $.daowenMenu({ width: 100, items: [
        ...
        _applyDrag: function (tabItemDom)
        {
            ...
            var drag = $(tabItemDom).daowenDrag(
            {
        ...
        {
            if (!$.fn.daowenDrag) return; //需要daowenDrag的支持
            var g = this, p = this.options;
            if (value)
            {
                if (g.drags) return;
...
(function ($)
{
    $.fn.daowenTextBox = function ()
    {
        return $.daowenui.run.call(this, "daowenTextBox", arguments);
    };

    $.fn.daowenGetTextBoxManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetTextBoxManager", arguments);
    };

    $.daowenDefaults.TextBox = {
        onChangeValue: null,
        width: null,
        disabled: false,
        value: null,     //初始化值 
    ...
    $.daowenui.controls.TextBox = function (element, options)
    {
        $.daowenui.controls.TextBox.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.TextBox.daowenExtend($.daowenui.controls.Input, {
        __getType: function ()
        {
            return 'TextBox'
        },
        ...
        _init: function ()
        {
            $.daowenui.controls.TextBox.base._init.call(this);
            var g = this, p = this.options;
            if (!p.width)
            {
...
{
    //气泡,可以在制定位置显示
    $.daowenTip = function (p)
    {
        return $.daowenui.run.call(null, "daowenTip", arguments);
    };

    //在指定Dom Element右侧显示气泡
    //target：将daowenui对象ID附加上
    $.fn.daowenTip = function (options)
    {
        this.each(function ()
        {
            var p = $.extend({}, $.daowenDefaults.ElementTip, options || {});
            ...
            if (p.auto || options == undefined)
            {
                ...
                }).bind('mouseout.tip', function ()
                {
                    ...
                    var tipmanager = $.daowenui.managers[this.daowenuitipid];
                    if (tipmanager)
                    {
                        tipmanager.remove();
                    }
            ...
            {
                if (p.target.daowenuitipid) return;
                p.x = $(this).offset().left + $(this).width() + (p.distanceX || 0);
                p.y = $(this).offset().top + (p.distanceY || 0);
                p.x = p.x || 0;
                p.y = p.y || 0;
        ...
        return $.daowenui.get(this, 'daowenuitipid');
    };
    //关闭指定在Dom Element(附加了daowenui对象ID,属性名"daowenuitipid")显示的气泡
    $.fn.daowenHideTip = function (options)
    {
        return this.each(function ()
        {
            ...
            {
                //如果是lable，将查找指定的input，并找到daowenui对象ID
            ...
            var tipmanager = $.daowenui.managers[target.daowenuitipid];
            if (tipmanager)
            {
                tipmanager.remove();
            }
    ...
    $.fn.daowenGetTipManager = function ()
    {
        return $.daowenui.get(this);
    };
    ...
    //在指定Dom Element右侧显示气泡,通过$.fn.daowenTip调用
    $.daowenDefaults.ElementTip = {
        distanceX: 1,
        distanceY: -3,
        auto: null,
        removeTitle: true
    ...
    $.daowenui.controls.Tip = function (options)
    {
        $.daowenui.controls.Tip.base.constructor.call(this, null, options);
    };
    $.daowenui.controls.Tip.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'Tip';
        },
        ...
        _render: function ()
        {
            ...
            if (p.appendIdTo)
            {
                p.appendIdTo.attr("daowenTipId", g.id);
            }
            if (p.target)
            {
                $(p.target).attr("daowenTipId", g.id);
                p.target.daowenuitipid = g.id;
            }
        ...
        remove: function ()
        {
            ...
            {
                this.options.appendIdTo.removeAttr("daowenTipId");
            }
            if (this.options.target)
            {
                $(this.options.target).removeAttr("daowenTipId");
                this.options.target.daowenuitipid = null;
            }
            this.tip.remove();
        }
    });
...
(function ($)
{
    ...
    $.fn.daowenToolBar = function (options)
    {
        return $.daowenui.run.call(this, "daowenToolBar", arguments);
    };

    $.fn.daowenGetToolBarManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetToolBarManager", arguments);
    };

    $.daowenDefaults.ToolBar = {};

    $.daowenMethos.ToolBar = {};

    $.daowenui.controls.ToolBar = function (element, options)
    {
        $.daowenui.controls.ToolBar.base.constructor.call(this, element, options);
    };
    $.daowenui.controls.ToolBar.daowenExtend($.daowenui.core.UIComponent, {
        __getType: function ()
        {
            return 'ToolBar';
        },
...
(function ($)
{
    $.fn.daowenTree = function (options)
    {
        return $.daowenui.run.call(this, "daowenTree", arguments);
    };

    $.fn.daowenGetTreeManager = function ()
    {
        return $.daowenui.run.call(this, "daowenGetTreeManager", arguments);
    };
    ...
    $.daowenui.controls.Tree = function (element, options)
    {
        $.daowenui.controls.Tree.base.constructor.call(this, element, options);
    };

    $.daowenui.controls.Tree.daowenExtend($.daowenui.core.UIComponent, {
        _init: function ()
        {
            $.daowenui.controls.Tree.base._init.call(this);
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.daowenWindow = function (options)
    {
        return l.run.call(null, "daowenWindow", arguments, { isStatic: true });
    };

    $.daowenWindow.show = function (p)
    {
        return $.daowenWindow(p);
    };

    $.daowenDefaults.Window = {
        showClose: true,
        showMax: true,
        showToggle: true,
        showMin: true,
    ...
    l.controls.Window.daowenExtend(l.core.Win, {
        __getType: function ()
        {
            return 'Window';
        },
        ...
        _render: function ()
        {
            ...
            if (p.url)
            {
                ...
                else
                {
                    ...
                    var framename = "daowenuiwindow" + l.windowCount++;
            ...
            if ($.fn.daowenDrag)
            {
                g.draggable = g.window.drag = g.window.daowenDrag({ handler: '.l-window-header-inner', onStartDrag: function ()
                {
                    g.active();
                }, onStopDrag: function ()
                {
            ...
            if ($.fn.daowenResizable)
            {
                g.resizeable = g.window.resizable = g.window.daowenResizable({
